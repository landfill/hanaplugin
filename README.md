...
- **의존성**: Chrome, Node.js v22+ (EXTERNAL 캡처 시). Figma MCP는 플러그인에 번들되어 자동 등록, 최초 1회 OAuth 인증 필요
...
| | pdf-to-markdown | tagging-definition | tabular-to-chart | figma-capture |
|---|---|---|---|---|
| **Python** | 필수 (3.10+) | PPT/엑셀 생성 시 필수 | 스크립트 모드 시 필수 | 불필요 |
| **Java** | 필수 (11+) | 불필요 | 불필요 | 불필요 |
| **Node.js** | 불필요 | 불필요 | 불필요 | v22+ (EXTERNAL 캡처 시) |
| **Chrome** | 불필요 | 불필요 | 불필요 | 필수 |
| **pip 패키지** | `opendataloader-pdf` (pip, 대화형 설치) | `python-pptx` `openpyxl` `Pillow` (venv 자동) | `openpyxl` (venv 자동, xlsx만) | 불필요 |
| **미설치 시 자동 설치** | Python·Java·pip 모두 시도 | Python 시도, pip 패키지 자동 | Python 안내, pip 패키지 자동 | 안내만 제공 |
| **Python 없을 때 폴백** | 자동 설치 시도 → 실패 시 중단 | 마크다운 표 채팅 출력 | LLM이 HTML/Mermaid 직접 생성 | 해당 없음 |
